...
async function fetchFeedJson(url, options = {}) {
...
async function getSocialFeed(response) {
  const now = Date.now();
  ...
  if (places.status === 'fulfilled' && places.value.length) {
    ...
    feed.status.places = 'live_openstreetmap';
  }
  localSocialFeedCache = { time: now, feed };
